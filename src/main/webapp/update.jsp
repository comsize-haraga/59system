...
<tr>
<td>タスク名</td>
<td><input type="text" name="task_name" style="width:300px" value =  <%= bean.getTask_name() %>></td>
</tr>

<tr>
<td>カテゴリ情報</td>
<td><select name="category_name">
<option value = <%=bean.getCategory_id() %>> <%=bean.getCategory_name() %> </option>
<option value = 1 >新商品A：開発プロジェクト </option>
<option value = 2 >既存商品B：改良プロジェクト </option>
</select></td></tr>

<tr><td>期限</td><td><input type = "date" name="limit_date" value = <%= bean.getLimet_date()%>></td></tr>

<tr><td>担当者情報</td><td><input type="text" name="user_name" value = <%=bean.getUser_id()%> >
</td></tr>
...
<tr><td>メモ</td><td><input type="text" name="comment" style="width:300px" value =  <%= bean.getMemo() %>></td></tr>
<%} %>
</table>
<br>
